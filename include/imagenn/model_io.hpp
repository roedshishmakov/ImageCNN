...
/// \param[in] network сеть для сохранения
/// \param[in] path путь к файлу модели
...
/// \param[in,out] network сеть, в которую загружаются веса
/// \param[in] path путь к файлу модели
/// \throw PathError если файл не найден
/// \throw ValidationError при несоответствии формата или структуры сети
...
/// \param[in] model модель для сохранения
/// \param[in] path путь к файлу модели
/// \throw PathError если файл не удаётся открыть на запись
void save_model(const Model& model, const std::string& path);

/// \brief Загружает веса в модель из текстового файла.
/// \param[in,out] model модель, в которую загружаются веса
/// \param[in] path путь к файлу модели
/// \throw PathError если файл не найден
/// \throw ValidationError при несоответствии формата или структуры модели
...
/// \throw PathError если файл не найден
/// \throw ValidationError если файл содержит некорректные данные
